...
noncomputable def is_frame (old new : Arr Int) (lo : Int) : Prop :=
  ∀ i, (0 <= i && i < lo) -> new i = old i

noncomputable def sort_is_perm (old new : Arr Int) (lo hi : Int) : Prop :=
  is_perm old new lo hi /\ is_frame old new lo
...
theorem swap_is_frame (old new: Arr Int) (lo i j: Int) :
  lo <= i ->  lo <= j -> is_frame old new lo -> is_frame old (swap new i j) lo
